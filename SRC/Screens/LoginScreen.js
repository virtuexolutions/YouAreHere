...
import {View, ToastAndroid, Platform, Alert, ActivityIndicator, ScrollView} from 'react-native';
...
const LoginScreen = () => {
  ...
  const LoginUser = async () => {
    ...
    console.log("🚀 ~ file: loginScreen.js:59 ~ LoginUser ~ response:", response?.data)
    ...
    if (response?.data?.success) {
      ...
      dispatch(setPreferencesSet(response?.data?.data?.user_info?.preferences.length > 0 ? true : false));
    }
